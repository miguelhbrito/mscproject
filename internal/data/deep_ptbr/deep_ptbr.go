...
type DeepPtbrInt interface {
	Save(Question, *log.Logger) error
	List() (Questions, error)
	GetIndexLastQ() (LastQuestion, error)
	GetLinkDeepPtbr() (LinkDeepPtbr, error)
	SaveLastQ(questionNumber int) error
}

type DeepPtbrPostgres struct{}

func (da DeepPtbrPostgres) Save(q Question, log *log.Logger) error {
	...
	// `SELECT title FROM deep_question_ptbr WHERE title = $1`
	sqlStatementVerify, _, _ := goqu.From("deep_question_ptbr").
	...
	if err != nil {
		if err == sql.ErrNoRows {
			...
			log.Println("New question DeepAnswers-PtBR, answer and comments was successfully saved into db!")
			return nil
		}
	}
	...
	// `SELECT * FROM deep_answer_ptbr WHERE question_id = $1`
	sqlAnsVerify, _, _ := goqu.From("deep_answer_ptbr").Where(goqu.C("question_id").Eq(qsVerify.Id)).ToSQL()
	...
	for idx, answerNew := range ans {
		// If the key exists
		if answerValue, ok := asnwersMap[idx]; ok {

			if answerNew.AnswerContent == answerValue.AnswerContent {

				log.Println("Old answer, check in new comments")
				var commentVerify []Comment
				// `SELECT * FROM deep_comment_ptbr WHERE answer_id = $1`
				sqlCommentVerify, _, _ := goqu.From("deep_comment_ptbr").Where(goqu.C("answer_id").Eq(answerValue.Id)).ToSQL()
				...
				if err := da.saveCommentExistedQuestion(commentsMap, answerValue, answerNew, db, log); err != nil {
					return err
				}
			}
		} else {
			log.Println("Inserting new answer in an existing question:", qsVerify.Id, answerNew)

			ansIdExisted, err := da.saveAnswer(qsVerify.Id, answerNew, db, log)
			if err != nil {
	...
	log.Println("Successfully saved the question DeepAnswers-PtBR, answer and comments into db!")
	return nil
}

func (da DeepPtbrPostgres) saveQuestion(qs Question, db *sql.DB, log *log.Logger) (int, error) {
	var questionId int
	log.Println("Inserting new DeepAnswers PtBR question into DB")
	sqlStatement :=
		`INSERT INTO "deep_question_ptbr" (title, question, category, tags, votes, author, type, created_at, points) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING id`
	err := db.QueryRow(sqlStatement, qs.Title, qs.Question, qs.Category, pq.Array(qs.Tags), qs.Votes, qs.Author, qs.Type, qs.DataCreated, qs.Points).Scan(&questionId)
	if err != nil {
		log.Println("Error to insert an new deepAnswers question ptbr question into db, err: ", err)
		return 0, err
	}
...
func (da DeepPtbrPostgres) saveAnswer(qsId int, answer Answer, db *sql.DB, log *log.Logger) (int, error) {
	var answerId int
	sqlStatement :=
		`INSERT INTO deep_answer_ptbr (question_id, answer_content, votes, author, type, created_at, points) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING id`
	err := db.QueryRow(sqlStatement, qsId, answer.AnswerContent, answer.Votes, answer.Author, answer.Type, answer.DataCreated, answer.Points).Scan(&answerId)
	if err != nil {
		log.Print("Error to insert an deepAnswers ptbr answer into db, err: ", err)
		return 0, err
	}
...
func (da DeepPtbrPostgres) saveComment(ansId int, comment Comment, db *sql.DB, log *log.Logger) error {
	sqlStatement :=
		`INSERT INTO deep_comment_ptbr (answer_id, commentary, author, type, created_at) VALUES ($1, $2, $3, $4, $5)`
	_, err := db.Exec(sqlStatement, ansId, comment.Commentary, comment.Author, comment.Type, comment.DataCreated)
	if err != nil {
		log.Println("Error to insert an deepAnswers ptbr commentary into db, err: ", err)
		return err
	}
	return nil
}

func (da DeepPtbrPostgres) saveCommentExistedQuestion(
	...
	for idxC, comment := range answerNew.Comments {
...
func (da DeepPtbrPostgres) List() (Questions, error) {
	db, err := dbConn.ConnectDB()
	if err != nil {
		log.Println("Error to connect to db:", err)
		return nil, err
	...
	sqlStatement := `SELECT id, title, question, category, tags, votes, author, type, created_at, points FROM deep_question_ptbr`
	rows, err := db.Query(sqlStatement)
	if err != nil {
		logz.Error().Err(err).Msg("Error to get all deep_question_ptbr from db")
		return nil, err
	}

	for rows.Next() {
		var q Question
		err := rows.Scan(&q.Id, &q.Title, &q.Question, &q.Category, pq.Array(&q.Tags), &q.Votes, &q.Author, &q.Type, &q.DataCreated, &q.Points)
		if err != nil {
			logz.Error().Err(err).Msg("Error to extract result from row")
		}

		sqlStatementAnswers := `SELECT id, question_id, answer_content, votes, author, type, created_at, points FROM deep_answer_ptbr WHERE question_id = $1`
		answerRows, err := db.Query(sqlStatementAnswers, q.Id)
		if err != nil {
			logz.Error().Err(err).Msg("Error to get all deep_answer_ptbr from db")
			return nil, err
		}
		...
		for answerRows.Next() {
			...
			sqlStatementComment := `SELECT id, answer_id, commentary, author, type, created_at FROM deep_comment_ptbr WHERE answer_id = $1`
			commentRows, err := db.Query(sqlStatementComment, a.Id)
			if err != nil {
				logz.Error().Err(err).Msg("Error to get all deep_comment_ptbr ptbr from db")
				return nil, err
			}
...
func (da DeepPtbrPostgres) GetIndexLastQ() (LastQuestion, error) {
	db, err := dbConn.ConnectDB()
	if err != nil {
		log.Println("Error to connect to db:", err)
	...
	idDeepPtbr := "deep_ptbr"

	var lq LastQuestion
	//SELECT id, question_number FROM last_question WHERE id = $1
	sqlStatement, _, _ := goqu.From("last_question").Select("id", "question_number").Where(goqu.C("id").Eq(idDeepPtbr)).ToSQL()
...
func (da DeepPtbrPostgres) GetLinkDeepPtbr() (LinkDeepPtbr, error) {
	db, err := dbConn.ConnectDB()
	if err != nil {
		log.Println("Error to connect to db:", err)
		return LinkDeepPtbr{}, err
	}
	defer func() {
		if err := db.Close(); err != nil {
			log.Printf("Error to close database connection: %v", err)
	...
	idDeepPtbr := "deep_ptbr"
	var linkDeepPtbr LinkDeepPtbr

	//SELECT id, link FROM link_scraper WHERE id = $1
	sqlStatement, _, _ := goqu.From("link_scraper").Select("id", "link").Where(goqu.C("id").Eq(idDeepPtbr)).ToSQL()
	result := db.QueryRow(sqlStatement)
	err = result.Scan(&linkDeepPtbr.Id, &linkDeepPtbr.Link)
	if err != nil {
		if err == sql.ErrNoRows {
			logz.Error().Err(err).Msg("Error, no rows in result")
			return LinkDeepPtbr{}, err
	...
	return linkDeepPtbr, nil
}

func (da DeepPtbrPostgres) SaveLastQ(questionNumber int) error {
	db, err := dbConn.ConnectDB()
	if err != nil {
		log.Println("Error to connect to db:", err)
		return err
	...
	sqlStatement, _, _ := goqu.Update("last_question").Set(goqu.C("question_number").Eq(questionNumber)).Where(goqu.C("id").Eq(idDeepPtbr)).ToSQL()
