...
type DeepEsEsInt interface {
	Save(Question, *log.Logger) error
	List() (Questions, error)
	GetIndexLastQ() (LastQuestion, error)
	GetLinkDeepEses() (LinkDeepEses, error)
	SaveLastQ(questionNumber int) error
}

type DeepEsesPostgres struct{}

func (da DeepEsesPostgres) Save(q Question, log *log.Logger) error {
	...
	// `SELECT title FROM deep_question_eses WHERE title = $1`
	sqlStatementVerify, _, _ := goqu.From("deep_question_eses").
	...
	log.Println("DeepAnswers EsES question already exist in DB!")
	log.Println("Check in new answers or comments!")

	var answersVerify []Answer
	// `SELECT * FROM deep_answer_eses WHERE question_id = $1`
	sqlAnsVerify, _, _ := goqu.From("deep_answer_eses").Where(goqu.C("question_id").Eq(qsVerify.Id)).ToSQL()
	...
	for idx, answerExisted := range ans {
		...
		if answerValue, ok := asnwersMap[idx]; ok {
			...
			if answerExisted.AnswerContent == answerValue.AnswerContent {
				...
				// `SELECT * FROM deep_comment_eses WHERE answer_id = $1`
				sqlCommentVerify, _, _ := goqu.From("deep_comment_eses").Where(goqu.C("answer_id").Eq(answerValue.Id)).ToSQL()
...
func (da DeepEsesPostgres) saveQuestion(qs Question, db *sql.DB, log *log.Logger) (int, error) {
	var questionId int
	log.Println("Inserting new DeepAnswers Spanish-ES question into DB")
	sqlStatement :=
		`INSERT INTO "deep_question_eses" (title, question, category, tags, votes, author, type, created_at, points) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING id`
	...
	if err != nil {
		log.Println("Error to insert an new deepAnswers-spanish question question into db, err:", err)
		return 0, err
	}
...
func (da DeepEsesPostgres) saveAnswer(qsId int, answer Answer, db *sql.DB, log *log.Logger) (int, error) {
	var answerId int
	sqlStatement :=
		`INSERT INTO deep_answer_eses (question_id, answer_content, votes, author, type, created_at, points) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING id`
	...
	if err != nil {
		log.Println("Error to insert an deepAnswers-spanish answer into db, err:", err)
		return 0, err
	}
...
func (da DeepEsesPostgres) saveComment(ansId int, comment Comment, db *sql.DB, log *log.Logger) error {
	sqlStatement :=
		`INSERT INTO deep_comment_eses (answer_id, commentary, author, type, created_at) VALUES ($1, $2, $3, $4, $5)`
	_, err := db.Exec(sqlStatement, ansId, comment.Commentary, comment.Author, comment.Type, comment.DataCreated)
	if err != nil {
		log.Println("Error to insert an deepAnswers-spanish commentary into db, err:", err)
		return err
	}
...
func (da DeepEsesPostgres) saveCommentExistedQuestion(
...
func (da DeepEsesPostgres) List() (Questions, error) {
	db, err := dbConn.ConnectDB()
	if err != nil {
		log.Println("Error to connect to db:", err)
		return nil, err
	...
	// `SELECT id, title, question, category, tags, votes, author, type, created_at, points FROM deep_question_eses`
	sqlStatement, _, _ := goqu.From("deep_question_eses").
		Select("id", "title", "question", "category", "tags", "votes", "author", "type", "created_at", "points").ToSQL()
	rows, err := db.Query(sqlStatement)
	if err != nil {
		logz.Error().Err(err).Msg("Error to get all deep_question_eses from db")
		return nil, err
	}
	...
	for rows.Next() {
		...
		//`SELECT id, question_id, answer_content, votes, author, type, created_at, points FROM deep_answer_eses WHERE question_id = $1`
		sqlStatementAnswers, _, _ := goqu.From("deep_answer_eses").
		...
		if err != nil {
			logz.Error().Err(err).Msg("Error to get all deep_answer_eses from db")
			return nil, err
		}
		...
		for answerRows.Next() {
			...
			// `SELECT id, answer_id, commentary, author, type, created_at FROM deep_comment_eses WHERE answer_id = $1`
			sqlStatementComment, _, _ := goqu.From("deep_comment_eses").
			...
			if err != nil {
				logz.Error().Err(err).Msg("Error to get all deep_comment_eses from db")
				return nil, err
			}
...
func (da DeepEsesPostgres) GetIndexLastQ() (LastQuestion, error) {
	db, err := dbConn.ConnectDB()
	if err != nil {
		log.Println("Error to connect to db:", err)
	...
	idDeepEsEs := "deep_eses"

	var lq LastQuestion
	//SELECT id, question_number FROM last_question WHERE id = $1
	sqlStatement, _, _ := goqu.From("last_question").Select("id", "question_number").Where(goqu.C("id").Eq(idDeepEsEs)).ToSQL()
...
func (da DeepEsesPostgres) GetLinkDeepEses() (LinkDeepEses, error) {
	db, err := dbConn.ConnectDB()
	if err != nil {
		log.Println("Error to connect to db:", err)
		return LinkDeepEses{}, err
	}
	defer func() {
		if err := db.Close(); err != nil {
			log.Printf("Error to close database connection: %v", err)
	...
	idDeepEsEs := "deep_eses"
	var linkDeepEsEs LinkDeepEses

	//SELECT id, link FROM link_scraper WHERE id = $1
	sqlStatement, _, _ := goqu.From("link_scraper").Select("id", "link").Where(goqu.C("id").Eq(idDeepEsEs)).ToSQL()
	result := db.QueryRow(sqlStatement)
	err = result.Scan(&linkDeepEsEs.Id, &linkDeepEsEs.Link)
	if err != nil {
		if err == sql.ErrNoRows {
			logz.Error().Err(err).Msg("Error, no rows in result")
			return LinkDeepEses{}, err
	...
	return linkDeepEsEs, nil
}

func (da DeepEsesPostgres) SaveLastQ(questionNumber int) error {
	db, err := dbConn.ConnectDB()
	if err != nil {
		log.Println("Error to connect to db:", err)
		return err
