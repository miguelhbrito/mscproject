...
import (
	...
	ha "github.com/miguelhbrito/mscproject/internal/data/ha_ptbr"
)
...
func NewCore(haptbr ha.HAPtbrInt, log *log.Logger) HAInt {
	return core{
		haptbr: haptbr,
		log:    log,
	}
...
func (cS core) getLastIndex() (int, error) {
	lq, err := cS.haptbr.GetIndexLastQ()
	if err != nil {
...
func (cS core) getLinkHAPtbr() (string, error) {
	link, err := cS.haptbr.GetLinkHAPtbr()
	if err != nil {
		cS.log.Println("error to get HA ptbr link, err:", err.Error())
		return "", err
	}
...
func (cS core) Scrapper(done chan commons.Done, errorCh chan commons.Error, log *log.Logger) {
	...
	cS.log.Println("Starting new scraper ha-ptBR!")
	...
	c.OnHTML("div.qa-main ", func(h *colly.HTMLElement) {
		...
		if !reflect.ValueOf(question).IsZero() && ((question.Category == "Ciência e tecnologia") ||
			(question.Category == "Privacidade e segurança") ||
			(question.Category == "Hacking") ||
			(question.Category == "Darknet e Tor") ||
			(question.Category == "Mercado negro") ||
			(question.Category == "Criptomoedas") ||
			(question.Category == "Scam e scammers") ||
			(question.Category == "Links") ||
			(question.Category == "Programação") ||
			(question.Category == "CC's")) {
	...
	if lastQuestionInt > 0 {
		err = cS.haptbr.SaveLastQ(lastQuestionInt)
		if err != nil {
	...
	cS.log.Println("Successfully scraped hidden-answers ptBR")

	done <- commons.Done{
		Msg:    "Successfully got scraped HA ptbr",
		Status: 200,
	}
}

func (cS core) List() (ha.Questions, error) {
	qts, err := cS.haptbr.List()
	if err != nil {
		return nil, err
	}
